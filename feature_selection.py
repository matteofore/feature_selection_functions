...
def feature_selection_phi(data, kpi, threshold_phi=0.05):
    selected_features = []
    for feature in data.columns:
        if feature == kpi:
            continue
        contingency_table = pd.crosstab(data[kpi], data[feature])
...
def driver_selection_cont(data_set, kpi, threshold_r2, threshold_phi, p_value, drivers):
    num_drivers = []
    cat_drivers = []
    ...
    if data_set[kpi].nunique() != 2:
        #for continous variables -> Pearson correlation  
        drivers_kpi = [kpi] + num_drivers
    ...
    #the target is binary
        ...
        #for categorical variables -> Phi co-efficient
        drivers_kpi = [kpi] + cat_drivers
        final_drivers_cat = feature_selection_phi(data_set[drivers_kpi], kpi, threshold_phi)
